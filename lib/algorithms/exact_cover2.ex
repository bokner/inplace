defmodule InPlace.ExactCover2 do
  @moduledoc """
  Implementation of exact cover via dancing cells.
  Based on https://youtu.be/PUJ_XdmSDZw?si=41ySCBvOdoNCV-zR
  ...
  def init(options) do
    ...
    item_header =
      SparseSet.new(num_items, mapper: fn _set, header_idx ->
          ## Header pointers.
    ...
    item_lists_ll =
      SparseSet.new(entry_count + num_items, mapper: fn _set, item -> Enum.at(option_lists, item - 1) end)
    ...
    SparseSet.each(
        ...
        fn p ->
          ...
          SparseSet.each(
            item_lists_ll,
            fn s ->
               ## TODO: apply to items starting with item_top
              Array.put(top, s, p)
            end
          )
        end
    ...
    ##
    option_lists_ll =
      SparseSet.new(entry_count, mapper: fn _set, opt ->
        Enum.at(item_lists, opt - 1)
      end)

    ## TODO: connect option lists with item lists
  ...
  defp iterate_row(
    ...
    SparseSet.each(
      ...
      ## TODO: iterate over options that are associated with row pointer
    )
  end
