defmodule InPlace.LinkedList do
  ...
  def get(list, idx) when is_integer(idx) do
  ...
  def add_first(%{next: next, refs: refs} = list, data) when is_integer(data) do
    ...
    old_head = get_and_update_head(list, allocated)

    ## Update pointer to next element with current head
    Array.put(next, allocated, old_head)
  end

  def add(%{next: pointers, refs: refs} = list, idx, data) when is_integer(data) do
    cond do
      idx > size(list) -> {:error, {:no_index, idx}}
      empty?(list) -> add_first(list, data)
      true ->
  ...
  defp get_and_update_head(%{capacity: capacity, next: next} = list, pointer) do
    current_head = head(list)
    Array.put(next, capacity + 1, pointer)
    current_head
  end
